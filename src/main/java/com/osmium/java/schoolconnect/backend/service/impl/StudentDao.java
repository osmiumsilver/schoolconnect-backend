...
public class StudentDao extends BaseDao {
    ...
    public ArrayList<User> query_all_student() {     // 获取所有学生的信息，用ArrayList返回
        sql = "select * from student order by sno;";
        ArrayList<User> results = new ArrayList<>();
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                User student = new User(rs.getInt("sno"), rs.getString("sname"), rs.getString("ssex"), rs.getByte("sage"), rs.getInt("clno"));
                results.add(student);
            }
